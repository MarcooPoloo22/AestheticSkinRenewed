    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(
          `backend/admin_dashboard_backend/delete_surgery.php`,
          {
            method: "DELETE",
            credentials: "include",
            headers: {
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <button
            onClick={() =>
              setActivePage({ page: "ManageSurgeryEdit", surgery: row })
            }
...
const ManageSurgeryEdit = ({ setActivePage, activePage, fetchSurgeries }) => {
  ...
  const fetchBranches = async () => {
    try {
      ...
      if (!result.success || !Array.isArray(result.data)) {
        throw new Error("Invalid branches format");
      }
  ...
  const fetchStaff = async () => {
    try {
      ...
      const surgeryStaff = result.data.filter(
        (staff) =>
          staff.is_surgery_staff === "1" || staff.is_surgery_staff === 1
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "backend/admin_dashboard_backend/update_surgery.php",
        {
          method: "POST",
          credentials: "include",
          body: formPayload,
      ...
      Swal.fire({
        ...
        text:
          error.message ||
          "Failed to update surgery. Please check the console for details.",
        icon: "error",
        confirmButtonText: "OK",
      });
    }
  ...
  return (
    <div className="faq-main-container">
      <button
        onClick={() => setActivePage({ page: "SurgeryAppointments" })}
...
const ManageSurgeryAdd = ({ setActivePage, activePage, fetchSurgeries }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `backend/admin_dashboard_backend/add_surgery.php`,
        {
          method: "POST",
          credentials: "include",
          body: formPayload,
...
const SurgeryAppointments = () => {
  ...
  const fetchSurgeries = async () => {
    try {
      const response = await fetch(
        "backend/admin_dashboard_backend/fetch_surgeries.php"
      );
      if (!response.ok) {
  ...
  const filteredData = surgeries.filter(
    (item) =>
          ...
          <div className="faq-header">
            ...
            <div className="faq-header-actions">
              <button
                ...
                style={{ width: "163px" }}
                onClick={() => setActivePage({ page: "ManageSurgeryAdd" })}
              >
                + Add Appointment
              </button>
