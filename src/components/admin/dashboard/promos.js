...
const FAQTable = ({ setActivePage, activePage, data }) => {
  ...
  const columns = [
    ...
    {
      name: 'Start Date',
      selector: row => row.startDate.toLocaleString(),
      sortable: true,
    },
    {
      name: 'End Date',
      selector: row => row.endDate.toLocaleString(),
      sortable: true,
    },
    {
      name: 'Action',
        ...
        <form className="faq-edit">
          ...
          <div className="date-row">
            <div className="date-input">
              ...
              <input type="date" className="answerInput" id="startDateInput" name="startDateInput" required />
            </div>
            <div className="date-input">
              <label className="answerLabel" htmlFor="endDateInput">End Date</label>
              <input type="date" className="answerInput" id="endDateInput" name="endDateInput" required />
            </div>
          </div>
          <button className="button-ManageFAQEdit">Save</button>
        </form>
...
const FAQs = () => {
  ...
  const filteredData = initialData.filter(item =>
    ...
    item.startDate.toLocaleString().toLowerCase().includes(searchText.toLowerCase()) ||
    item.endDate.toLocaleString().toLowerCase().includes(searchText.toLowerCase())
  );
        ...
        <div className="faq-main-container">
          <div className="faq-header">
            ...
            <div className="faq-header-actions">
              <button className="add-faq-button" style={{width: "110px"}} onClick={() => setActivePage("ManageFAQAdd")}>
                + Add Promo
              </button>
