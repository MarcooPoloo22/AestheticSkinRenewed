    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(
          `backend/admin_dashboard_backend/delete_promo.php`,
          {
            method: "DELETE",
            credentials: "include",
            headers: {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <button
            onClick={() => setActivePage({ page: "ManageFAQEdit", promo: row })}
...
const ManageFAQEdit = ({ setActivePage, activePage, fetchPromos }) => {
  ...
  const startTime = promo.start_date
    ? promo.start_date.split("T")[1].substring(0, 5)
    : "00:00";
  const endDate = promo.end_date ? promo.end_date.split("T")[0] : "";
  const endTime = promo.end_date
    ? promo.end_date.split("T")[1].substring(0, 5)
    : "00:00";
  ...
  const fetchBranches = async () => {
    try {
      ...
      setBranches(result.data); // ✅ Correct
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `backend/admin_dashboard_backend/update_promo.php`,
        {
          method: "POST",
          credentials: "include",
  ...
  return (
    <div className="faq-main-container">
      <button
        onClick={() => setActivePage({ page: "FAQs" })}
      ...
      <div className="faq-edit">
        ...
        <form className="faq-edit" onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="questionLabel" htmlFor="nameInput">
              Name
            </label>
            <input
          ...
          <div className="form-group">
            <label className="answerLabel" htmlFor="fileInput">
              File
            </label>
            <input
              className="answerInput"
          ...
          <div className="form-group">
            <label className="questionLabel" htmlFor="duration">
              Duration (hours)
            </label>
            <select
              className="questionInput"
...
const ManageFAQAdd = ({ setActivePage, activePage, fetchPromos }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `backend/admin_dashboard_backend/add_promo.php`,
        {
          method: "POST",
          credentials: "include",
          body: formPayload,
...
const FAQs = () => {
  ...
  const fetchPromos = async () => {
    try {
      const response = await fetch(
        "backend/admin_dashboard_backend/fetch_promos.php"
      );
      if (!response.ok) {
  ...
  const filteredData = promos.filter(
    (item) =>
          ...
          <div className="faq-header">
            ...
            <div className="faq-header-actions">
              <button
                ...
                style={{ width: "110px" }}
                onClick={() => setActivePage({ page: "ManageFAQAdd" })}
              >
                + Add Promo
              </button>
